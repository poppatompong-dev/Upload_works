...
- Video archive รวม: `E:\ExamVideos\PR-2569` แยก `original\` และ `mp4\`
...
│  ├─ exporter.js            candidate/global manifest, video archive และ backup
...
│  ├─ archive-videos.js      copy วิดีโอทั้งหมดไป EXAM_VIDEO_ARCHIVE_ROOT
...
| `EXAM_VIDEO_ARCHIVE_ROOT` | `E:\ExamVideos\PR-2569` | root รวมไฟล์วิดีโอผู้สอบทุกคน แยกไฟล์ต้นฉบับและ MP4 |
...
- video archive ที่ `EXAM_VIDEO_ARCHIVE_ROOT\original` และ `EXAM_VIDEO_ARCHIVE_ROOT\mp4`
...
- ตรวจ: `dbWritable`, `backupWritable`, `videoArchiveWritable`, `ffmpegAvailable`, `ffprobeAvailable`, `rosterCount`, `disk`, `warnings`
...
- ล้างโฟลเดอร์ `submissions/`, `_tmp/`, video archive `original/`, `mp4/` และ backup `submissions/` ตามค่า default
...
21. copy วิดีโอไป video archive รวมบน `EXAM_VIDEO_ARCHIVE_ROOT` โดยแยก `original` และ `mp4`
22. backup candidate folder
23. broadcast realtime
24. Candidate เปิด preview แล้ว confirm
25. ระบบ set submission เป็น `confirmed`, เขียน manifest อีกครั้ง และ backup อีกครั้ง
...
Video archive layout ภายใต้ `EXAM_VIDEO_ARCHIVE_ROOT`:

```text
E:\ExamVideos\PR-2569
├─ original\
│  └─ 01_07101001_<candidate-name>\
│     └─ 01_<original-name>_<file-id>.<ext>
└─ mp4\
   └─ 01_07101001_<candidate-name>\
      └─ 01_<original-name>_<file-id>.mp4
```

ระบบจะ copy วิดีโอไป archive นี้อัตโนมัติหลัง verify/transcode สำเร็จ และสามารถรันย้อนหลังได้ด้วย:

```powershell
npm run archive:videos
```
...
- ตรวจพื้นที่ว่าง drive หลัก, backup และ Drive E สำหรับ video archive มากกว่า 20 GB
...
- สำรองทั้ง `D:\ExamSubmissions\PR-2569`, `C:\ExamSubmissionsBackup\PR-2569` และ `E:\ExamVideos\PR-2569`
...
- 2026-05-06 - Codex - เพิ่ม video archive รวมบน Drive E: ค่าเริ่มต้น `EXAM_VIDEO_ARCHIVE_ROOT=E:\ExamVideos\PR-2569`, ระบบ copy วิดีโอผู้สอบหลัง verify/transcode ไป `original/` และ `mp4/` แยกตาม candidate folder, เพิ่มคำสั่ง `npm run archive:videos` สำหรับรันย้อนหลัง, เพิ่ม health/check/reset/test coverage และอัปเดตเอกสาร storage layout - `server/config.js`, `server/index.js`, `server/exporter.js`, `server/upload.js`, `server/state.js`, `server/reset.js`, `server/routes.js`, `src/types.ts`, `scripts/archive-videos.js`, `package.json`, `tests/core.test.js`, `tests/upload-flow.test.js`, `SYSTEM_DOCUMENTATION.md`
...
- แก้ export/backup/video archive: `server/exporter.js`
...
- อย่าแก้ storage root โดยไม่ตรวจ backup/export/video archive path
